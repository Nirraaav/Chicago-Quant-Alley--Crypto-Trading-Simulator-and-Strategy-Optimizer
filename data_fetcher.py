...
tickers_response.raise_for_status() #checks if exception was raised i.e. status is not 200
tickers_data = tickers_response.json()["result"]
btc_price = None
for item in tickers_data:
    if item["symbol"] == "BTCUSDT":
        btc_price = float(item["spot_price"]) #current spot price of underlying asset: i.e. the current market price of the underlying asset
        break
